...
if __name__ == "__main__":
    for item in TEST_DATA:
        ...
        tps = get_true_positives(item[1], entities)
        fps = get_false_positives(item[1], entities)
        fns = get_false_negatives(item[1], entities)

        print(calculate_f_measure(calculate_precision(tps, fps), calculate_recall(tps, fns)))
